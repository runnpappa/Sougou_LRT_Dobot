def setup1(cap0,cap1,Size):
    ...
    if len(corners0)>0: # もしマーカーが検出されたら
        ...
        s.trans_mat1 = cv2.getPerspectiveTransform(marker_coordinates0,true_coordinates) # 任意の四角形から別の任意の四角形への変換 引数のデータ型はfloat32でないといけないらしい
    ...
    if len(corners1)>0: # もしマーカーが検出されたら
            ...
            s.trans_mat2 = cv2.getPerspectiveTransform(marker_coordinates1,true_coordinates) # 任意の四角形から別の任意の四角形への変換
