def setup2(s,cap1,cap2,Size):
    ...
    s.m1 = np.empty((4,2)) # 要素数2の配列を4行作る
    s.m1[0] = Left_end_point
    s.m1[1] = Right_end_point
    s.m1[2] = Right_start_point
    s.m1[3] = Left_start_point

    marker_coordinates = np.float32(s.m1) # 四隅のマーカーの中心座標(カメラに対して斜めの面)
    true_coordinates   = np.float32([[0,0],[width,0],[width,height],[0,height]]) # カメラに対して平行な面
    s.trans_mat1_a = cv2.getPerspectiveTransform(marker_coordinates,true_coordinates) # 任意の四角形から別の任意の四角形への変換 引数のデータ型はfloat32でないといけないらしい
